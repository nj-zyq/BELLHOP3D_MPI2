...
SUBROUTINE BellhopCore
  ...
  INTEGER              :: IBPvec( 1 ), ibp, iBeamWindow2, irz, itheta, isx, isy, isz, iRec, ir, iloop, id
  ...
  CASE ( 'C', 'S', 'I' )        ! TL calculation
     ...
     ALLOCATE ( rdcP( Pos%Ntheta, NRz_per_range, Pos%NRr ), Stat = IAllocStat )
  ...
  Source_z: DO isz = 1, Pos%NSz         ! loop over source z-coordinate
     ...
     Source_x: DO isx = 1, Pos%Nsx      ! loop over source x-coordinate
        ...
        Source_y: DO isy = 1, Pos%Nsy   ! loop over source y-coordinate
           ...
           !AzimuthalAngle: DO iloop = 1,ceiling(real(Angles%Nbeta)/real(nprocs)) 
           !   ibeta = (iloop - 1) * nprocs + myrank + 1
              ...
              IF ( Angles%iSingle_beta == 0 .OR. ibeta == Angles%iSingle_beta ) THEN    ! Single beam run?
                ...
                CALL FLUSH( PRTFile )
                 ...
                 DeclinationAngle: DO ialpha = 1, Angles%Nalpha
                    id = ibeta * Angles%Nalpha + ialpha
                    IF ( mod(id, nprocs) == myrank ) THEN
                 ...
                 END IF
              ENDIF   ! closes iSingle test
            !ENDIF !IF ( ibeta .LE. Angles%Nbeta)
    ...
    call MPI_REDUCE( P, rdcP,Pos%Ntheta*NRz_per_range*Pos%NRr , MPI_COMPLEX,MPI_SUM, 0, MPI_COMM_WORLD, ierr)
              ...
              DO irz = 1, Pos%NRz
                 RcvrBearing: DO itheta = 1, Pos%Ntheta
                    iRec = 10 + ( isx    - 1 ) * Pos%Nsy * Pos%Ntheta * Pos%NSz * Pos%NRz + &
                                ...
                                ( isz    - 1 ) *                                  Pos%NRz + irz
                    WRITE( SHDFile, REC = IRec ) rdcP( itheta, irz, 1 : Pos%NRr )
